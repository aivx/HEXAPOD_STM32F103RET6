...
class Leg{
  ...
  public void IK(float dx, float dy){
     ...
     ey = sin(E) * a + sy;
     int y = (int)degrees(E);
     this.y = (int)constrain(map(y == int(y) ? y:90, 210, 470,0,180),0,180);
...
void onBluetoothDataEvent(String who, byte[] data) { 
  for(byte i = 0; i<6; i++){
    ...
    bt.writeToDeviceName(device_name, convert(values));
  }
...
public void onStart() {
  super.onStart();  
   ...
   cp5.enableShortcuts();
   cp5.addButton("START_BT").setPosition(50, 20).setSize(50,30);
   cp5.addButton("MODE_1").setPosition(150, 20).setSize(50,30);
   cp5.addButton("MODE_2").setPosition(250, 20).setSize(50,30);
   cp5.addButton("MODE_3").setPosition(350, 20).setSize(50,30);
   cp5.addButton("MODE_4").setPosition(450, 20).setSize(50,30);
   cp5.addButton("MODE_5").setPosition(550, 20).setSize(50,30);
...
void onPause() {
  if(bt.isStarted()){
    //bt.disconnectDevice(device_name);
    bt.stop(); 
  }
}
